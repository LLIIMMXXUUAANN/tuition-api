...
- **`app/features/agent/lg/utils.py`** — `extract_text(msg)`: shared helper that extracts plain text from an `AIMessage` / `AIMessageChunk`, handling both `str` and `list` content formats. Imported by `supervisor.py` and `stream_adapter.py` (was previously duplicated in both).
