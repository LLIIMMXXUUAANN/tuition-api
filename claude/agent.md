...
- `update_student` auto-syncs Calendar + Drive when `class_schedule` changes: three branches — (1) schedule cleared → delete events + blank Drive doc; (2) `calendar_event_ids` exist → nuke-and-repave via `update_weekly_class_events` + `update_student_meet_doc` in parallel; `effective_meet_link` is recovered directly from Google so `google_meet_link` does not need to be set in the DB; (3) no existing IDs → create fresh events via `create_weekly_class_events`; Google failures are non-fatal in all branches (returned as `google_warning`)
...
- `err_msg(err, fallback)` — use everywhere instead of inlining error strings (`app/shared/errors.py`)
...
- `generate_payment_message` defaults to next calendar month (MYT) when `month`/`year` are omitted; delegates all calculation to `build_payment_message()` from `app/features/payment/service.py`; returns `{ message, month, year, monthName }`; DB row is validated through `_PaymentStudentRow` (a private Pydantic model) before field access — consistent with `_StudentFeeRow` in `get_fee_summary`
